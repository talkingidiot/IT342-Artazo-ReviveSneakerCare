    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}
      ...
      <div style={{ background: "#fff", borderRadius: "18px", width: "100%", maxWidth: "440px", boxShadow: "0 24px 80px rgba(0,0,0,0.22)" }}>
        <div style={{ padding: "32px 36px 24px", borderBottom: "1px solid #f0ebe4", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "26px", fontWeight: 700, color: "#3a2e1e", margin: 0 }}>
            {isRegister ? "Create Account" : "Welcome Back"}
          </h2>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "20px", color: "#999" }}>✕</button>
        </div>
        <form onSubmit={isRegister ? handleRegister : handleLogin} style={{ padding: "28px 36px 36px", display: "flex", flexDirection: "column", gap: "18px" }}>
          {error && (
            <div style={{ background: "#fee", border: "1px solid #fcc", borderRadius: "8px", padding: "12px", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "13px", color: "#c33" }}>
              {error}
            </div>
          )}
          {success && (
            <div style={{ background: "#efe", border: "1px solid #cfc", borderRadius: "8px", padding: "12px", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "13px", color: "#3c3" }}>
              {success}
            </div>
          )}
          ...
          {isRegister && (
            <div>
              <label style={{ display: "block", fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "14px", fontWeight: 600, color: "#3a2e1e", marginBottom: "7px" }}>Full Name</label>
          ...
          <div>
            ...
            <input type="password" placeholder={isRegister ? "At least 8 characters" : "Enter your password"} value={form.password}
          ...
          <button
            ...
            disabled={loading}
            style={{
              background: loading ? "#ccc" : "#8B7355", color: "#fff", border: "none", borderRadius: "8px",
              padding: "14px", fontSize: "12px", fontWeight: 700, letterSpacing: "2px",
              cursor: loading ? "default" : "pointer", fontFamily: "'Cormorant Garamond', Georgia, serif", marginTop: "4px",
              opacity: loading ? 0.6 : 1,
            }}
            onMouseEnter={(e) => !loading && (e.target.style.background = "#6b5a3e")}
            onMouseLeave={(e) => !loading && (e.target.style.background = "#8B7355")}
          >
          ...
          <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "13px", color: "#999", textAlign: "center", margin: 0 }}>
            ...
            <span onClick={toggleMode} style={{ color: "#8B7355", cursor: "pointer", fontWeight: 700 }}>
...
function Navbar({ cartCount, onCartOpen, onLoginOpen, isLoggedIn, user, onLogout }) {
...
function Dashboard({ user, onLogout, cart, onAddToCart, onRemoveFromCart, showCart, onCartOpen, onCartClose }) {
  const [scrolled, setScrolled] = useState(false);
  ...
  return (
    <div style={{ margin: 0, padding: 0, boxSizing: "border-box" }}>
      ...
      <AuthoritySection />
      <Footer />
      {showCart && <CartSidebar cart={cart} onClose={onCartClose} onRemove={onRemoveFromCart} />}
    </div>
  );
}
